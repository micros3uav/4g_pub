...
*/
 ...
 if(i<20)
 {
  ...
  wp_push_client.call(req_wp,resp_wp);
      ...
      if (resp_wp.success)
       // ROS_INFO_STREAM("WyPoints Transferreded %d ",wp_transfered);
       // ROS_INFO_STREAM("bool %s",resp_wp.success);
      i++;
  }
   */
  ...
  if (ros::service::exists("/mavros/mission/push",false)){
    if (ros::service::call("/mavros/mission/push",req_wp,resp_wp)){
      std::cout<<"Success!"<<std::endl;
      if (resp_wp.success)
        std::cout<<"Transferred "<<resp_wp.wp_transfered<<std::endl;
      else
